...
const createMarkup = async (componentDir) => {
  const srcTemplate = await readFile(
    join(componentsDir, componentDir, "template.hbs"),
    "utf-8"
  );
  const compiledTemplate = handlebars.compile(srcTemplate);
  const writePath = join(buildDir, componentDir, "temlplate.html");
  await writeFile(writePath, compiledTemplate({ artist: "Beatles" }), {
    flag: "w",
  });
};
  ...
  components.forEach((componentDir) => {
    ...
    esbuild
      .build({
        bundle: true,
        entryPoints: [
          join(componentsDir, componentDir, "script.mjs"),
          join(componentsDir, componentDir, "style.scss"),
        ],
        outdir: outputDir,
        plugins: [
          sassPlugin(),
...
buildComponents().then(console.log).catch(console.error);
